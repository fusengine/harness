...
/**
 * Fixture: a delivered corpus (README + THREE tokens files — C5 raised the
 * `full`-mode threshold to 3) and an active design agent. `setup()` only
 * provisions the 3rd file (c-recode) ON DISK — it does NOT read it, so each
 * test body decides WHEN that read happens relative to its own assertions.
 */
function setup() {
  ...
  mkdirSync(join(root, "c-recode"), { recursive: true });
  ...
  writeFileSync(join(root, "c-recode", "tokens-c.md"), "# c");
...
test("designGate with an injected corpus: reads advance the state, jointure enforced at write", () => {
  ...
  // C5: the corpus completes with a 3rd tokens read (full mode needs 3).
  gate(ev("post", "Read", join(root, "c-recode", "tokens-c.md")));
...
test("designGate on apply_patch add (event.files): add content IS the document — garbage blocked, plugin root protected", () => {
  const { cache, root, ev, gate } = setup();
  for (const f of ["README.md", "a-recode/tokens-a.md", "b-recode/tokens-b.md", "c-recode/tokens-c.md"]) gate(ev("post", "Read", join(root, f)));
  gate(ev("post", "mcp__fuse-browser__browser_screenshot"));
  gate(ev("post", "mcp__fuse-browser__browser_screenshot"));
